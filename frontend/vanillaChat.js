...
async function handleGoogleSignIn() {
	try {
		if (firebaseConfig.apiKey === "YOUR_API_KEY") {
			alert("Firebase is not configured yet. Fallback to mock credentials.");
			return;
		}
...
function showSignupView(e) {
	...
	DOM.signupView.classList.add('active');
}

function showLoginView(e) {
	if (e) e.preventDefault();
	DOM.signupView.classList.remove('active');
...
function updateSearchingStatus() {
	...
	if (isSearching && hint) {
		hint.textContent = '🌐 Searching the web for latest information...';
		hint.style.color = 'var(--accent)';
		hint.style.fontWeight = '500';
	} else if (hint) {
		hint.textContent = 'GT can make mistakes. Consider checking important information.';
		hint.style.color = '';
...
function toggleWebSearch() {
	...
	if (DOM.webSearchBtn) {
		if (webSearchEnabled) {
			DOM.webSearchBtn.classList.add('active');
			DOM.webSearchBtn.title = 'Web Search ON';
			DOM.userInput.placeholder = 'Search the web and ask GT...';
		} else {
			DOM.webSearchBtn.classList.remove('active');
			DOM.webSearchBtn.title = 'Enable Web Search';
			DOM.userInput.placeholder = 'Message GT...';
		}
	}
}
...
DOM.showSignupBtn.addEventListener('click', showSignupView);
DOM.showLoginBtn.addEventListener('click', showLoginView);
DOM.logoutBtn.addEventListener('click', handleLogout);
if (DOM.googleLoginBtn) DOM.googleLoginBtn.addEventListener('click', handleGoogleSignIn);
if (DOM.googleSignupBtn) DOM.googleSignupBtn.addEventListener('click', handleGoogleSignIn);
...
function init() {
	// Set default model to Llama 3.3 70B (Unlimited)
	if (DOM.modelSelect) {
		DOM.modelSelect.value = '@cf/meta/llama-3.3-70b-instruct-fp8-fast';
	}
	checkAuth();
}
